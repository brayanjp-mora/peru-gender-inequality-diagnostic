...
\section{The Randomized ``Worst-off" Algorithm}
...
\begin{enumerate}
    ...
    \item Recursive Optimization: The algorithm calculates the income gap for all subgroups within the random subset. It selects the ``Worst-off" branch (lowest median income + highest inequality) and repeats the process on that subgroup. 
    \item Pathfinding \& Replication: This process is repeated with 500 replications. The final output identifies the ``Most Frequent Worst-off Paths" –the specific combinations of identities (e.g., Indigenous $\to$ Daily Pay $\to$ Rural) that consistently appear at the bottom of the socioeconomic ladder across all simulations. 
\end{enumerate}
...
\begin{enumerate}
    \item Intersectional Profiling: Deep-dive profiling of the top 3 ``Worst-Off" groups identifies by the algorithm (analyzing their specific barriers to health, trust, and rights.)
